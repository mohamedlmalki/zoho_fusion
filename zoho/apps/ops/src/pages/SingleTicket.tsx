...
const SingleTicket: React.FC<SingleTicketProps> = ({ onAddProfile, onEditProfile, onDeleteProfile }) => {
  ...
  // --- MODIFICATION: Added missing functions ---
  const handleClearFailures = () => {
    if (socket && activeProfileName) {
        if (window.confirm("Are you sure you want to clear all failure alerts? This cannot be undone.")) {
  ...
  const handleExportFailures = () => {
    ...
    // Create CSV content
    const header = "Ticket Number,Email,Subject,Reason,Error Message,Department,Assignee\n";
    const csvContent = emailFailures.map(f => {
        const ticket = f.ticketNumber || 'N/A';
        ...
        const dept = f.departmentName || 'N/A';
        const assignee = f.assignee?.name || 'Unassigned';
        return [ticket, email, subject, reason, error, dept, assignee].join(',');
    }).join('\n');
    ...
    link.setAttribute("download", "email_failures.csv");
