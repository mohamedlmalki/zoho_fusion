...
export default function ContactManagerBigin() {
  ...
  const { data: contacts = [], isLoading: isLoadingContacts, refetch: refetchContacts } = useQuery({
    queryKey: ['/api/bigin/contacts', selectedAccountId],
  ...
  // --- CLIENT-SIDE BATCH DELETE LOGIC ---
  const handleBulkDelete = async () => {
    ...
    const BATCH_SIZE = 50; // Bigin Chunk Size
    ...
    try {
        for (let i = 0; i < total; i += BATCH_SIZE) {
            ...
            await apiRequest('DELETE', `/api/bigin/contacts/${selectedAccountId}`, { ids: batch });
      ...
      <div className="form-card">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <h3 className="text-lg font-semibold">Bigin Contact Manager</h3>
                ...
                <Dialog open={isDeleteDialogOpen} onOpenChange={(open) => !isDeleting && setIsDeleteDialogOpen(open)}>
                    ...
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                {isDeleting 
                                    ? "Please wait while we delete the selected contacts from Bigin..."
