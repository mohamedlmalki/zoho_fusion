...
interface TaskResultsDisplayProps {
  ...
  // NEW: Lifted state to keep checkboxes saved
...
export const TaskResultsDisplay: React.FC<TaskResultsDisplayProps> = ({
  ...
  // NEW: Lightning-Fast Frontend Pagination
  ...
  // Reset pagination when filtering or switching projects
  useEffect(() => {
    setCurrentPage(1);
  }, [filterText, selectedProjectId]);

  // Generate the currently visible 100 items to stop browser lag
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      // Select ALL items across all pages instantly
    ...
    <div className="space-y-4">
        ...
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-4 sm:space-y-0 pb-4">
            ...
            <div className="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-2 w-full sm:w-auto">
                ...
                <Button variant="outline" size="icon" onClick={fetchTasks} title="Refresh Tasks" className="w-full sm:w-10">
                    <RefreshCw className="h-4 w-4 mr-2 sm:mr-0" />
                    <span className="sm:hidden">Refresh Server</span>
                </Button>
            </div>
        </CardHeader>
                        ...
                        <table className="w-full relative">
                            ...
                            <tbody className="divide-y divide-border">
                                {paginatedTasks.map((task, localIndex) => {
                                    ...
                                    // Calculate global index for the "#" column
