...
// This isolates the 1-second re-renders so the main heavy component doesn't lag.
const TimerDisplay = ({ timerState }: { timerState: any }) => {
  const [tick, setTick] = useState(0);

  useEffect(() => {
    // Only start interval if timer is running (has start, no end, not paused)
  ...
  const timeString = useMemo(() => {
    ...
    let now = Date.now();
    // If finished, freeze at end time
            ...
            {selectedAccountId && (
                <div className="flex justify-around text-center mt-auto border-t pt-4">
                    <div>
                        ...
                        <TimerDisplay timerState={allFormsData.timers?.[`crm-${selectedAccountId}`]} />
                    </div>
                    <div>
