...
const handleStartBulkInvoice = async (socket, data) => {
    // Added customEmailMethod to distinguish between 'invoice' and 'contact' API options
    const { 
        ...
        for (let i = 0; i < emails.length; i++) {
            ...
            try {
                const invoiceData = {
                    ...
                    // --- CHANGED HERE ---
                    notes: body,    // Store Email BODY in 'notes'
                    terms: subject  // Store Email SUBJECT in 'terms' (Terms & Conditions)
                };

                // OPTION 1: SEND DEFAULT INVOICE
                if (sendDefaultEmail) {
                    // Overriding default template with app input
                ...
                if (sendDefaultEmail) {
                    ...
                    if (apiMessage.includes("error while sending the invoice")) {
                    ...
                    else if (isEmailed === false) {
                        socket.emit('invoiceResult', {
                            rowNumber,
                            email,
                            stage: 'complete',
                            success: false,
                            details: "Invoice created but EMAIL FAILED (Daily Limit likely reached).",
                            invoiceResponse: invoiceResponsePayload,
                            contactResponse: contactResponsePayload,
                            emailResponse: { success: false, fullResponse: { message: "Detected is_emailed: false in response." } },
                            profileName: selectedProfileName
                        });
                    ...
                    socket.emit('invoiceResult', {
                        ...
                        email, 
                        stage: 'invoice', 
                        details: 'Invoice created. Sending custom email...',
                        invoiceResponse: invoiceResponsePayload,
                        contactResponse: contactResponsePayload,
                        profileName: selectedProfileName
                    });
            ...
            if (sendCustomEmail) {
                try {
                    ...
                    if (customEmailMethod === 'contact') {
                        // OPTION 3: SEND VIA CONTACT API (Guaranteed NO Attachment)
                        try {
                            await makeApiCall('post', `/v1/invoices/${invoiceId}/status/sent`, null, activeProfile, 'inventory');
                        } catch(e) { console.log("Status update skipped/failed"); }
                        ...
                        const emailApiUrl = `/v1/contacts/${contactId}/email`;
                        
                        console.log('\x1b[31m%s\x1b[0m', '🔥🔥🔥 [BULK] OPTION 3: CONTACT API (NO ATTACHMENT)');

                        const emailApiResponse = await makeApiCall('post', emailApiUrl, emailData, activeProfile, 'inventory');
                        ...
                        socket.emit('invoiceResult', {
                            ...
                            details: `Custom email sent (Contact API) for Invoice #${invoiceNumber}.`,
                        ...
                        // OPTION 2: SEND VIA INVOICE API (Likely has Attachment)
                        const emailData = {
                        ...
                        const emailApiUrl = `/v1/invoices/${invoiceId}/email`;

                        console.log('\x1b[31m%s\x1b[0m', '🔥🔥🔥 [BULK] OPTION 2: INVOICE API');

                        const emailApiResponse = await makeApiCall('post', emailApiUrl, emailData, activeProfile, 'inventory', queryParams);
                        const emailSendResponsePayload = { success: true, fullResponse: emailApiResponse.data };
                        
                        socket.emit('invoiceResult', {
                            rowNumber, email, stage: 'complete', success: true,
                            details: `Custom email sent (Invoice API) for Invoice #${invoiceNumber}.`,
                            invoiceNumber: invoiceNumber,
                            emailResponse: emailSendResponsePayload,
                            contactResponse: contactResponsePayload,
                            invoiceResponse: invoiceResponsePayload,
...
const handleSendSingleInvoice = async (data) => {
    ...
    try {
        ...
        // --- ROBUST CHECK FOR SINGLE INVOICE ---
        if (sendDefaultEmail) {
             const isEmailed = invoiceResponse.data.invoice?.is_emailed;
             const apiMessage = invoiceResponse.data.message || "";
             ...
             if (apiMessage.includes("error while sending the invoice")) {
                 return {
                    success: false,
                    message: "Invoice created but Error Sending Email: " + apiMessage,
             ...
             if (isEmailed === false) {
                 return { 
                    success: false, 
                    message: "Invoice created but EMAIL FAILED (Daily Limit likely reached or email disabled).",
                    fullResponse: fullResponse
                };
             }
        }
        ...
        if (sendCustomEmail) {
            if (customEmailMethod === 'contact') {
                // OPTION 3: SEND VIA CONTACT API (NO ATTACHMENT)
                try {
                    await makeApiCall('post', `/v1/invoices/${invoiceId}/status/sent`, null, activeProfile, 'inventory');
                } catch(e) { console.log("Status update skipped/failed"); }
                ...
                const emailData = {
                    to_mail_ids: [email],
                    subject: subject,
                    body: `${body} <br><br>Invoice Number: ${invoiceNumber}`,
                };
                const emailApiUrl = `/v1/contacts/${contactId}/email`;
                console.log('\x1b[31m%s\x1b[0m', '🔥🔥🔥 [SINGLE] OPTION 3: CONTACT API');

                const emailApiResponse = await makeApiCall('post', emailApiUrl, emailData, activeProfile, 'inventory');
                fullResponse.email = emailApiResponse.data;

            } else {
                // OPTION 2: SEND VIA INVOICE API
                const emailData = {
                    send_from_org_email_id: false,
                    to_mail_ids: [email],
                    subject: subject,
                ...
                console.log('\x1b[31m%s\x1b[0m', '🔥🔥🔥 [SINGLE] OPTION 2: INVOICE API');
        ...
        const message = sendDefaultEmail 
            ? `Invoice ${invoiceNumber} created and default email sent.`
            : sendCustomEmail
            ? `Invoice ${invoiceNumber} created and custom email sent (${customEmailMethod}).`
            : `Invoice ${invoiceNumber} created without sending email.`;
