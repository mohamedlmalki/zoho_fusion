...
import { 
    ...
    Search, FileText, BarChart3, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Eye, AlertOctagon, RotateCcw 
} from 'lucide-react';
...
const CustomModuleBulk: React.FC<CustomModuleBulkProps> = ({ 
    ...
    useEffect(() => {
        ...
        if (moduleName) {
            if (formData.moduleApiName !== moduleName || !formData.availableFields?.length) {
                console.log("Auto-fetching fields for:", moduleName);
                updateFormData({ moduleApiName: moduleName });
                setIsFetchingFields(true);
                socket.emit('fetchModuleFields', { 
    ...
    useEffect(() => {
        ...
        return () => { socket.off('moduleFieldsResult', handleFieldsResult); };
    ...
    const handleStartJob = () => {
        ...
        // --- FIXED: Variable Names Matched to Server ---
        socket?.emit('startBulkCustomJob', {
            selectedProfileName: activeProfileName,
            moduleApiName: formData.moduleApiName,
            bulkField: formData.bulkField,  // Was bulkFieldApiName
            bulkData: formData.bulkData,    // Was bulkDataRaw
    ...
    const handlePauseResume = () => {
        ...
        if (currentJob.isPaused) {
            ...
            socket.emit('endJob', { profileName: activeProfileName, jobType: formData.moduleApiName });
            ...
            socket.emit('startBulkCustomJob', {
                selectedProfileName: activeProfileName,
                moduleApiName: formData.moduleApiName,
                bulkField: formData.bulkField, // Fixed
                bulkData: formData.bulkData,   // Fixed
                staticData: formData.staticData,
                delay: formData.delay,
                concurrency: formData.concurrency,
                stopAfterFailures: formData.stopAfterFailures,
            ...
            setJobs(prev => ({ 
                ...prev, 
                [activeProfileName]: { ...prev[activeProfileName], isPaused: false } 
            }));
            toast({ title: "Job Resumed", description: "Restoring connection..." });

        } else {
            // --- PAUSE LOGIC ---
            socket.emit('pauseJob', { profileName: activeProfileName, jobType: formData.moduleApiName });
            setJobs(prev => ({ 
                ...prev, 
    ...
    const handleExportTxt = () => {
        ...
        link.setAttribute("download", `custom_module_results_${new Date().toISOString().slice(0,10)}.txt`);
                ...
                <Card className="shadow-medium">
                    <CardHeader>
                        <CardTitle className="flex items-center space-x-2">
                            ...
                            <span>Custom Module Manager</span>
                        </CardTitle>
                        <CardDescription>
                    ...
                    <CardContent className="space-y-6">
                        ...
                        {!formData.moduleApiName && (
                            <Alert variant="destructive">
                                ...
                                <AlertDescription>
                                    This account does not have a <strong>Custom Module API Name</strong> set. 
                        ...
                        {isFetchingFields && (
                            <div className="flex justify-center items-center py-8 text-muted-foreground">
                                ...
                                <span>Fetching module fields...</span>
                            </div>
                        )}
                                    ...
                                    <div className="pt-4 border-t grid grid-cols-3 gap-4">
                                        <div>
                                            ...
                                            <Input 
                                                ...
                                                onChange={e => updateFormData({ delay: Number(e.target.value) })} 
                                                className="mt-1" 
                                                disabled={jobState.isProcessing}
                                                min={0}
                                            />
                                        ...
                                        <div>
                                            ...
                                            <Input 
                                                ...
                                                onChange={e => updateFormData({ concurrency: Number(e.target.value) })} 
                                                className="mt-1" 
                                                disabled={jobState.isProcessing}
                                                min={1}
                                                max={10}
                                            />
                                            <p className="text-[10px] text-muted-foreground mt-1">Threads (Max 10)</p>
                                        </div>
                                        <div>
                                            <Label className="flex items-center text-red-600/80" title="Stop after X failures">
                                                <AlertOctagon className="h-3 w-3 mr-1" /> Auto-Pause
                                            </Label>
                                            <Input 
                                                type="number" 
                                                value={formData.stopAfterFailures} 
                                                onChange={e => updateFormData({ stopAfterFailures: Number(e.target.value) })} 
                                                className="mt-1" 
                                                disabled={jobState.isProcessing}
                                                min={0}
                                                placeholder="0 (Off)"
                                            />
                                            <p className="text-[10px] text-muted-foreground mt-1">Failures to stop</p>
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-4 gap-2 pt-2 text-center">
                                        <div className="bg-muted/30 p-2 rounded">
                                            <div className="text-xl font-bold font-mono">{formatTime(jobState.processingTime)}</div>
                                        ...
                                        <div className="bg-green-500/10 p-2 rounded text-green-700">
                                            ...
                                            <div className="text-[10px] uppercase">Success</div>
                                        </div>
                                        <div className="bg-red-500/10 p-2 rounded text-red-700">
                        ...
                        <div className="pt-6 border-t space-y-4">
                            {!jobState.isProcessing ? (
                                ...
                                <div className="flex gap-4">
                                    {/* --- FIXED: Using handlePauseResume here --- */}
                                    ...
                                    <Button className="flex-1" variant="destructive" onClick={() => socket?.emit('endJob', { profileName: activeProfileName, jobType: formData.moduleApiName })}>
                        ...
                        <CardHeader className="pb-4">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center space-x-3 select-none">
                                    <Badge 
                                        variant="outline" 
                                        className={cn("cursor-pointer", statusFilter === 'all' ? "bg-primary text-primary-foreground" : "text-muted-foreground")}
                                    ...
                                    <Badge 
                                        variant="outline" 
                                        className={cn("bg-green-500/10 text-green-600 cursor-pointer border-transparent hover:border-green-200", statusFilter === 'success' && "ring-2 ring-green-500")}
                                        onClick={() => setStatusFilter(statusFilter === 'success' ? 'all' : 'success')}
                                    >
                                        <CheckCircle2 className="h-3 w-3 mr-1" /> {successCount} Success
                                    </Badge>
                                    {errorCount > 0 && (
                                        <Badge 
                                            variant="destructive" 
                                            className={cn("bg-destructive/10 cursor-pointer border-transparent", statusFilter === 'error' && "ring-2 ring-destructive")}
                            ...
                            <CardDescription>
                                {jobState.isProcessing ? 
                                    `Processing... ${completedCount} / ${jobState.totalToProcess} complete.` : 
                                    `Completed.`}
                            </CardDescription>
                        </CardHeader>
                                ...
                                <table className="w-full">
                                    ...
                                    <tbody className="bg-card divide-y divide-border">
                                        {paginatedResults.length === 0 ? (
                                            <tr><td colSpan={5} className="p-8 text-center text-muted-foreground">No results match your filter.</td></tr>
                                        ) : (
                                            paginatedResults.map((result) => (
                                                <tr key={result.rowNumber} className={result.stage === 'complete' && !result.success ? 'bg-destructive/5' : ''}>
                                                    ...
                                                    <td className={`p-3 text-sm ${result.stage === 'complete' && !result.success ? 'text-destructive' : 'text-muted-foreground'}`}>
                                                        {result.details}
                                                    </td>
                                                    <td className="p-3 text-center text-xs text-muted-foreground">{formatTimestamp(result.timestamp)}</td>
                                                    <td className="p-3 text-center">
                                                        <div className="flex items-center justify-center space-x-2">
                                                            {result.stage === 'processing' ? <Loader2 className="h-4 w-4 animate-spin text-muted-foreground"/> : (
                                                                result.success ? <CheckCircle2 className="h-4 w-4 text-green-500" /> : <XCircle className="h-4 w-4 text-destructive" />
                                                            )}
                                                                ...
                                                                <Dialog>
                                                                    ...
                                                                    <DialogContent className="max-w-2xl">
                                                                        ...
                                                                        <pre className="mt-2 max-h-[60vh] overflow-y-auto rounded-md bg-muted p-4 text-xs font-mono">
                                                                            {JSON.stringify(result.response || result.fullResponse, null, 2)}
                                                                        </pre>
                                                                    </DialogContent>
                                                                </Dialog>
                                                            )}
